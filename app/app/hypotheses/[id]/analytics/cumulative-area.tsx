...
export function CumulativeAreaChart({
  ...
  const tabs = useMemo(() => {
    ...
    return [
      { name: '1w', data: last7.length > 0 ? last7 : series },
      { name: '2w', data: last14.length > 0 ? last14 : series },
      { name: '4w', data: last30.length > 0 ? last30 : series },
      { name: 'All', data: series },
    ]
  }, [series])

  const totals = series[series.length - 1] || { Visitors: 0, Signups: 0 }
      ...
      <div className='p-4 md:p-6'>
        <Tabs value={selected} onValueChange={setSelected}>
          <div className='grid md:flex md:items-start md:justify-between'>
            <ul role='list' className='order-2 mt-4 flex flex-wrap items-center gap-x-8 gap-y-3 md:order-1 md:mt-0'>
              <li>
              ...
              <li>
                <div className='flex items-center space-x-2'>
                  <span className='size-3 shrink-0 rounded-sm bg-blue-500 dark:bg-blue-500' aria-hidden />
                  <p className='font-semibold text-gray-900 dark:text-gray-50'>
            ...
            <div className='order-1 md:order-2'>
              <TabsList variant='solid' className='w-full md:w-fit'>
                {tabs.map((tab) => (
                  <TabsTrigger key={tab.name} value={tab.name} className='w-full justify-center py-1 md:w-fit md:justify-start'>
                    {tab.name}
                  </TabsTrigger>
                ))}
              </TabsList>
